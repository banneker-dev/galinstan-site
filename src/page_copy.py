...
STAGE_2 = [
    ...
    Line(id="dep-body-2", status=APPROVED, approved_on="2026-09-22", text="**What you buy.** Galinstan is built to run on hardware you already know how to procure. We size each deployment against named, purchasable machines, measure the figures on the machine rather than model them, and publish the specification rather than quote it on request."),
    Line(id="dep-body-3", status=APPROVED, approved_on="2026-09-22", text="**What we are to your register.** We license Galinstan annually with support, patches and calibration. Supervisory guidance treats a software licence that carries ongoing support as an ICT service, so we expect to appear in your register of information as a provider. What changes is not the entry. It is that none of your data reaches us, and no part of the analysis depends on us being reachable."),
